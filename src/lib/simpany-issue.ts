...
async function loadSource(
  ...
  if (input.transactionId != null) {
    const [txn] = await db
      ...
      .limit(1);
    if (!txn) throw new SimpanyPreviewError(`找不到交易 #${input.transactionId}`);
  ...
  if (input.subscriptionId != null) {
    if (!input.subscriptionPeriod || !/^\d{4}-\d{2}-\d{2}$/.test(input.subscriptionPeriod)) {
    ...
    const [sub] = await db
      ...
      .limit(1);
    if (!sub) throw new SimpanyPreviewError(`找不到訂閱 #${input.subscriptionId}`);
    const schedule = await getSubscriptionSchedule(orgId, input.subscriptionId);
    const period = schedule?.periods.find((p) => p.periodStart === input.subscriptionPeriod);
    if (!schedule || !period) {
      throw new SimpanyPreviewError(
        `訂閱 #${input.subscriptionId} 沒有 ${input.subscriptionPeriod} 這一期（期別起日要對得上 get_subscription_schedule）`,
      );
    }
    if (period.invoicedOn) {
      warnings.push(`訂閱這一期已標記開發票日 ${period.invoicedOn}，可能重複開立`);
    }
    links.subscriptionId = input.subscriptionId;
    links.subscriptionPeriod = input.subscriptionPeriod;
...
async function duplicateWarnings(
  ...
  if (links.billingItemId != null) {
    ...
    for (const r of rows) out.push(`這個請款項目已經有發票 ${r.number ?? `#${r.id}`}`);
  }
  if (links.subscriptionId != null && links.subscriptionPeriod) {
    ...
    for (const r of rows) out.push(`這個訂閱期別已經有發票 ${r.number ?? `#${r.id}`}`);
  }
  ...
  if (links.partyId != null) {
    ...
    for (const r of rows) {
      out.push(`本系統 ${DUPLICATE_LOOKBACK_DAYS} 天內已有同客戶、同金額的發票 ${r.number ?? `#${r.id}`}（${r.date ?? "無日期"}），請確認不是重複開立`);
    }
  }
  try {
...
export async function previewSimpanyInvoice(
  ...
  if (src.partyId != null) {
    [party] = await db
      ...
      .from(parties)
      .where(and(eq(parties.organizationId, orgId), eq(parties.id, src.partyId)))
      .limit(1);
  }
  ...
  const buyerName = (input.buyer?.name ?? party?.name ?? "").trim();
  if (!buyerName) throw new SimpanyPreviewError("缺少買受人名稱（buyer.name）");
  ...
  // ---- foreign currency ----
  const srcForeign = src.currency.toUpperCase() !== "TWD" ? src.currency.toUpperCase() : null;
  const foreignCurrency = (input.foreignCurrency?.trim().toUpperCase() || srcForeign) ?? null;
  let foreign: ForeignInfo | null = null;
  if (foreignCurrency && foreignCurrency !== "TWD") {
  ...
  let zeroRateReason: SimpanyZeroTaxReason | null = null;
  let customsClearanceType: InvoicePreview["customsClearanceType"] = null;
  if (taxTreatment === "zero_rated") {
    ...
    if (reasons.length) {
      ...
      if (!hit) {
        throw new SimpanyPreviewError(
          `零稅率原因「${code}」不在 Simpany 的清單內：${reasons.map((r) => `${r.code} ${r.name}`).join("、")}`,
        );
      }
      zeroRateReason = hit;
    } else {
      if (!/^7\d$/.test(code)) throw new SimpanyPreviewError(`零稅率原因「${code}」格式不正確（應為 71–79）`);
      zeroRateReason = KNOWN_ZERO_TAX_REASONS.find((r) => r.code === code) ?? { code, name: "" };
      warnings.push("無法從 Simpany 取得零稅率原因清單，原因代碼未經驗證");
    }
    customsClearanceType = input.customsClearance ?? "NOT_VIA_CUSTOMS";
  ...
  // ---- items ----
  const rawItems: PreviewItemInput[] =
    input.items && input.items.length > 0
      ? input.items
      : baseTwd != null
        ? [{ name: src.itemName ?? "服務費", quantity: 1, price: baseTwd }]
        : [];
  ...
  const items = rawItems.map((it, i) => {
    ...
    if (!(quantity > 0)) throw new SimpanyPreviewError(`第 ${i + 1} 個品項數量要大於 0`);
    if (!(price > 0)) throw new SimpanyPreviewError(`第 ${i + 1} 個品項單價要大於 0`);
  ...
  if (!(amounts.total > 0)) throw new SimpanyPreviewError("發票總額必須大於 0");
  ...
  const summaryLine = [
    `${type} ${buyerName}${vat ? `（${vat}）` : ""}`,
    `${TAX_TREATMENT_LABEL[taxTreatment]}${zeroRateReason ? ` ${zeroRateReason.code}${zeroRateReason.name ? ` ${zeroRateReason.name}` : ""}` : ""}`,
    `未稅 ${amounts.untaxed} + 稅 ${amounts.tax} = 總計 NT$${amounts.total}`,
    foreign ? `${foreign.currency} ${foreign.amount} × ${foreign.exchangeRate}` : null,
    `${items.length} 個品項`,
  ]
    .filter(Boolean)
    .join("｜");
...
async function explainUnavailableDraft(orgId: string, draftId: number): Promise<never> {
  ...
  if (d.status === "issued") {
    throw new SimpanyPreviewError(
      `草稿 #${draftId} 已經開立過了${d.issuedInvoiceId ? `（發票 #${d.issuedInvoiceId}）` : ""}，不會重複開立`,
    );
  }
...
export async function issueSimpanyDraft(
  ...
  try {
    ...
    const definite =
      e instanceof SimpanyError && (e.kind === "validation" || e.kind === "business" || e.kind === "auth" || e.kind === "config");
    if (definite) {
  ...
  let detail: SimpanyReceiptDetail | null = null;
  const createdId = isObj(created) && typeof created.id === "string" ? created.id : null;
  try {
    if (createdId) detail = await client.getReceipt(createdId);
    else detail = parseDetail(created) ?? (await findJustIssued(client, body, total));
  } catch {
    detail = parseDetail(created);
  }
  if (!detail) {
    throw new SimpanyError(
      "business",
...
export async function resolveSimpanyReceiptId(
  ...
  const number = r.toUpperCase().replace(/[\s-]/g, "");
