/**
 * 隱私權政策的繁體中文條文。英文版在 ./content-en.tsx，兩份的章節與順序必須一致
  ...
  return (
    <>
      ...
      <section>
        ...
        <ul>
          <li>
            員工的銀行帳號與身分證字號<strong>以欄位層級加密後才存進資料庫</strong>（AES-GCM）。加密金鑰是部署環境的
            secret，不存在資料庫裡，所以單獨拿到資料庫內容看不到這些值。此外還有傳輸加密（HTTPS）、資料庫與物件儲存供應商提供的靜態加密，以及登入、組織隔離與角色權限。
          </li>
          <li>
            <strong>處處遮罩</strong>：列表、表單、發薪與撥款紀錄與 MCP 一律只顯示遮罩後的值 ——
            帳號只保留末 5 碼，身分證字號只保留前 3 碼，其餘以<code>*</code> 取代。
          </li>
          <li>
            唯一的例外是組織的<strong>擁有者或管理員</strong>在網頁上明確按下「顯示完整帳號」（薪轉作業需要），或編輯員工的身分證字號。
            一般成員看不到完整值，也不能修改員工資料。
          </li>
          <li>
            每一次「顯示完整帳號」都會寫進稽核紀錄（誰、何時、哪個帳戶），紀錄本身不含帳號。MCP 沒有顯示完整帳號的功能。
          </li>
          <li>本功能上線前以明文輸入的舊資料，會由一次性的搬移程序轉成加密欄位並清除明文。</li>
        </ul>
      </section>
      ...
      <section>
        ...
        <div className="panel">
          ...
          <h3>每一次存取都留紀錄</h3>
          <p>
            所有經由 MCP 的寫入，以及員工資料與員工帳戶的讀取，都會寫進稽核紀錄，通道標記為<code>mcp</code>，可在系統內查看。
          </p>
          <h3>token 存在哪裡</h3>
          <p>
