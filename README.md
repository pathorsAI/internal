...
Neon 專案 `team`（id `spring-poetry-26111843`），兩個分支：

| 分支 | ID | 用途 |
|---|---|---|
| **production** | `br-calm-violet-a1kqw6sp`（default / primary）| 正式資料，**資料的唯一真實來源** |
| **dev** | `br-bold-wildflower-a1fce385` | 開發用，**從 production 分出來的子分支**；本機 app 連這個 |
...
```bash
neonctl branches reset dev --parent --project-id spring-poetry-26111843
...
1. **看差異**確認 prod 缺什麼：
   ```bash
   neonctl branches schema-diff production dev --project-id spring-poetry-26111843
   ```
   （diff 是「讓 production 追上 dev」要做的改動。它是參考,不是可直接執行的腳本——實際要套的是你 step 2 寫好的 migration SQL。）
2. **先在 production 的拋棄式分支上試套**：
   ```bash
   neonctl branches create --name migrate-test --parent production --project-id spring-poetry-26111843
